...
test_api_host="https://openapi.cattrip.net/V1" 
prod_api_host="https://supplierapi.lanmao.cn/V1"
...
then 
  ...
  echo ' '
  REACT_APP_API_HOST=$prod_api_host REACT_APP_CDN_URL=$prod_cdn_host npm run build 
  echo -e "\033[32m 构建完成! \033[0m"
  npm run upload
  cd build
  tar zcvf ../deploy.tar.gz ./favicon.ico ./index.html
  cd ../
